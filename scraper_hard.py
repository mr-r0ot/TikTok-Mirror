...
def load_json(file, default=None):
    ...
    try:
        ...
        log(f"Error loading {file}: {e}. Starting empty.")
        return default if default is not None else {}
...
def add_and_save_posts(new_posts_dict):
    ...
    if changed:
        ...
        log(f"Incremental save: added {len(new_posts_dict)} new posts")
...
def fetch_messages_with_retry(channel_id, last_message_id=None, limit=MAX_LIMIT, channel_name=""):
    ...
    while True:
        try:
            response = requests.post(API_BASE, headers=HEADERS, json=body, timeout=30)
            response.raise_for_status()
            data = response.json()
            return data.get("messages", [])
        except requests.exceptions.Timeout:
            print("\n" + "="*60)
            print("⚠️  NETWORK TIMEOUT / تایم اوت شبکه")
            print("Please check your internet connection.")
            print("لطفاً اتصال اینترنت خود را بررسی کنید.")
            print("- If using mobile data, try toggling Airplane mode to change IP.")
            print("- If behind a modem/router, try restarting it.")
            print("- اگر از مودم استفاده می‌کنید، آن را خاموش و روشن کنید.")
            print("- اگر با گوشی متصل هستید، حالت هواپیما را فعال و غیرفعال کنید.")
            print("="*60)
        except requests.exceptions.ConnectionError:
            print("\n" + "="*60)
            print("⚠️  CONNECTION ERROR / خطای اتصال")
            print("Cannot reach the server. Check your network.")
            print("سرور در دسترس نیست. اتصال اینترنت را بررسی کنید.")
            print("="*60)
...
def get_all_messages_for_channel(channel_name, channel_id, full_archive=False):
    ...
    messages = fetch_messages_with_retry(channel_id, last_message_id=None, limit=MAX_LIMIT, channel_name=channel_name)
    if messages is None:
    ...
    for msg in messages:
        ...
        if mid:
            if newest_id < mid:
                newest_id = mid
            if oldest_id is None or oldest_id > mid:
                oldest_id = mid
    ...
    for msg in messages:
        ...
        if mid and (largest_existing is None or mid > largest_existing):
            ...
            if post:
                new_posts_dict[post["id"]] = post
    if new_posts_dict:
        add_and_save_posts(new_posts_dict)
        total_new += len(new_posts_dict)
        log(f"First batch: saved {len(new_posts_dict)} new posts (newest_id={newest_id})")
    
    # اگر full_archive=True، باید به سمت قدیمی‌ترها برویم تا جایی که به smallest_existing برسیم (یا به انتها)
    if full_archive:
        ...
        while True:
            ...
            if next_id < 1:
                break
            messages = fetch_messages_with_retry(channel_id, last_message_id=next_id, limit=MAX_LIMIT, channel_name=channel_name)
            if messages is None:
                break
            if not messages:
                break
            ...
            for msg in messages:
                mid = msg.get("message_id")
                if mid:
                    if new_oldest is None or new_oldest > mid:
                        new_oldest = mid
                    # فقط پست‌هایی که قبلاً نداریم را اضافه کن
            ...
            if new_posts_dict:
                add_and_save_posts(new_posts_dict)
                total_new += len(new_posts_dict)
                log(f"Older batch: saved {len(new_posts_dict)} new posts, now oldest_id={new_oldest}")
        ...
        # حالت به‌روزرسانی عادی (فقط جدیدترین‌ها)
        # دیگر نیازی به حلقه نیست، زیرا فقط جدیدترین دسته را گرفتیم
        pass
    
    # تعیین آخرین message_id ذخیره شده (جدیدترین)
...
def update_channel_new_posts(channel_name, channel_id, last_saved_id):
    log(f"Checking for new posts in '{channel_name}' (last saved id: {last_saved_id})")
    messages = fetch_messages_with_retry(channel_id, last_message_id=None, limit=MAX_LIMIT, channel_name=channel_name)
    if messages is None:
    ...
    if new_posts_dict:
        add_and_save_posts(new_posts_dict)
        log(f"Found and saved {len(new_posts_dict)} new posts")
        return len(new_posts_dict), newest_id
    else:
